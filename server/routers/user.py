...
async def get_auth_status(request: Request):
  ...
  try:
    ...
    return AuthenticationStatusResponse(
      authenticated=True, auth_mode=auth_mode, has_user_identity=True, user_id=user_identity.user_id
    )
  except Exception:
    # Token exists but is invalid - still in OBO mode
    return AuthenticationStatusResponse(
      authenticated=True, auth_mode=auth_mode, has_user_identity=False, user_id=None
    )
...
async def get_user_workspace(user_token: str = Depends(get_user_token)):
  ...
  return WorkspaceInfoResponse(
    ...
    workspace=WorkspaceInfo(name=workspace_info.workspace_name, url=workspace_info.workspace_url),
  )
