...
class UsageEventBatchRequest(BaseModel):
    ...
    def validate_batch_size(cls, v):
        ...
        if len(v) > 1000:
            raise ValueError(
                f'Batch size exceeds maximum of 1000 events (received: {len(v)})'
            )
        return v
...
class TimeSeriesDataPoint(BaseModel):
    ...
    avg_response_time_ms: Optional[float] = Field(None, description='Average response time (performance)')
