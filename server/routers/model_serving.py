...
async def list_endpoints(
  ...
  """
  logger.info('Listing model serving endpoints', user_id=user_id)
  ...
  try:
    ...
    logger.error(f'Error listing model serving endpoints: {str(e)}', exc_info=True, user_id=user_id)
    raise HTTPException(
      status_code=503,
      detail={
...
async def get_endpoint(
  ...
  Raises:
      ...
      503: Service unavailable
  """
  logger.info('Getting model serving endpoint', user_id=user_id, endpoint=endpoint_name)

  try:
    service = ModelServingService(user_token=user_token)
    endpoint = await service.get_endpoint(endpoint_name=endpoint_name)

    logger.info('Retrieved endpoint details', user_id=user_id, endpoint=endpoint_name)
    ...
    if 'not found' in error_str or 'does not exist' in error_str:
      logger.warning(f'Endpoint not found: {endpoint_name}', user_id=user_id)
      raise HTTPException(
        status_code=404,
        detail={
...
async def detect_endpoint_schema(
  ...
  Raises:
      ...
      503: Service unavailable
  """
  logger.info('Detecting schema for endpoint', user_id=user_id, endpoint=endpoint_name)

  try:
    service = SchemaDetectionService(user_token=user_token)
    result = await service.detect_schema(endpoint_name=endpoint_name, user_id=user_id)
    ...
    logger.warning(
      f'Schema detection not yet implemented: {str(e)}', user_id=user_id, endpoint=endpoint_name
    )
    raise HTTPException(
      status_code=503,
      detail={
    ...
    logger.error(
      f'Error detecting schema: {str(e)}', exc_info=True, user_id=user_id, endpoint=endpoint_name
    )
    raise HTTPException(
      status_code=503,
      detail={
...
async def invoke_model(
  ...
  try:
    ...
    if response.status == 'ERROR':
      raise HTTPException(
        ...
        detail={
          ...
          'technical_details': {'endpoint': request.endpoint_name, 'error': response.error_message},
          'retry_after': 30,
        },
      )
    ...
    logger.warning(
      f'Invalid inference request: {str(e)}', user_id=user_id, endpoint=request.endpoint_name
    )
    raise HTTPException(status_code=400, detail={'error_code': 'INVALID_INPUT', 'message': str(e)})
    ...
    raise HTTPException(
      ...
      detail={
        'error_code': 'MODEL_UNAVAILABLE',
        'message': 'Model service temporarily unavailable. Please try again in a few moments.',
        'technical_details': {'endpoint': request.endpoint_name, 'error': str(e)},
        'retry_after': 30,
      },
    )
...
async def get_inference_logs(
  request: Request, limit: int = 50, offset: int = 0, user_id: str = Depends(get_current_user_id)
):
  ...
  Raises:
      ...
      503: Service unavailable
  """
  logger.info('Retrieving inference logs', user_id=user_id, limit=limit, offset=offset)
  ...
  try:
    ...
    logger.error(f'Error retrieving inference logs: {str(e)}', exc_info=True, user_id=user_id)
    raise HTTPException(
      status_code=503,
      detail={
        'error_code': 'SERVICE_UNAVAILABLE',
