...
async def list_catalogs(
  ...
  """
  logger.info('Listing Unity Catalog catalogs', user_id=user_id)

  try:
    service = UnityCatalogService(user_token=user_token)
    catalogs = await service.list_catalogs(user_id=user_id)

    logger.info(f'Retrieved {len(catalogs)} catalogs', user_id=user_id, catalog_count=len(catalogs))

    return catalogs

  except Exception as e:
    logger.error(f'Error listing catalogs: {str(e)}', exc_info=True, user_id=user_id)
    raise HTTPException(
      status_code=503,
      detail={
...
async def list_schemas(
  ...
  Raises:
      ...
      503: Database unavailable (EC-002)
  """
  logger.info('Listing Unity Catalog schemas', user_id=user_id, catalog=catalog)

  try:
    service = UnityCatalogService(user_token=user_token)
    schemas = await service.list_schemas(catalog=catalog, user_id=user_id)
    ...
    logger.error(f'Permission denied: {str(e)}', user_id=user_id, catalog=catalog)
    raise HTTPException(
      status_code=403,
      detail={
        'error_code': 'CATALOG_PERMISSION_DENIED',
        'message': "You don't have access to this catalog.",
        'technical_details': {'catalog': catalog},
      },
    )
  except Exception as e:
    logger.error(f'Error listing schemas: {str(e)}', exc_info=True, user_id=user_id)
    raise HTTPException(
      status_code=503,
      detail={
        'error_code': 'DATABASE_UNAVAILABLE',
...
async def list_table_names(
  ...
  Raises:
      ...
      503: Database unavailable (EC-002)
  """
  logger.info('Listing Unity Catalog table names', user_id=user_id, catalog=catalog, schema=schema)

  try:
    service = UnityCatalogService(user_token=user_token)
    table_names = await service.list_table_names(catalog=catalog, schema=schema, user_id=user_id)
    ...
    logger.error(f'Permission denied: {str(e)}', user_id=user_id, catalog=catalog, schema=schema)
    raise HTTPException(
      status_code=403,
      detail={
        'error_code': 'CATALOG_PERMISSION_DENIED',
        'message': "You don't have access to this catalog/schema.",
        'technical_details': {'catalog': catalog, 'schema': schema},
      },
    )
  except Exception as e:
    logger.error(f'Error listing table names: {str(e)}', exc_info=True, user_id=user_id)
    raise HTTPException(
      status_code=503,
      detail={
        'error_code': 'DATABASE_UNAVAILABLE',
...
async def list_tables(
  ...
  try:
    service = UnityCatalogService(user_token=user_token)
    tables = await service.list_tables(catalog=catalog, schema=schema, user_id=user_id)
    return tables
    ...
    logger.error(f'Error listing tables: {str(e)}', exc_info=True, user_id=user_id)
    raise HTTPException(
      status_code=503,
      detail={
        'error_code': 'DATABASE_UNAVAILABLE',
...
async def query_table_get(
  ...
  try:
    ...
    result = await service.query_table(
      catalog=catalog, schema=schema, table=table, limit=limit, offset=offset, user_id=user_id
    )
    ...
    # Invalid parameters
    logger.warning(f'Invalid query parameters: {str(e)}', user_id=user_id)
    raise HTTPException(status_code=400, detail={'error_code': 'INVALID_QUERY', 'message': str(e)})

  except PermissionError as e:
    logger.error(
      f'Permission denied: {str(e)}', user_id=user_id, table=f'{catalog}.{schema}.{table}'
    )
    raise HTTPException(
      status_code=403,
      detail={
        ...
        'technical_details': {'catalog': catalog, 'schema': schema, 'table': table},
      },
    )
    ...
    logger.error(f'Error querying table: {str(e)}', exc_info=True, user_id=user_id)
    raise HTTPException(
      status_code=503,
      detail={
        'error_code': 'DATABASE_UNAVAILABLE',
