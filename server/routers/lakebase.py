...
async def get_preferences(
  request: Request, preference_key: str | None = None, user_id: str = Depends(get_current_user_id)
):
  ...
  """
  logger.info('Getting user preferences', user_id=user_id, preference_key=preference_key)

  try:
    # LakebaseService uses service principal for database connection (FR-011)
    service = LakebaseService()
    preferences = await service.get_preferences(user_id=user_id, preference_key=preference_key)

    logger.info(
      f'Retrieved {len(preferences)} preferences for user', user_id=user_id, count=len(preferences)
    )
    ...
    if 'Lakebase is not configured' in str(e):
      logger.warning(f'Lakebase not configured: {str(e)}', user_id=user_id)
      raise HTTPException(
        status_code=503,
        detail={
    ...
    # Other ValueError
    raise HTTPException(
      status_code=400, detail={'error_code': 'INVALID_REQUEST', 'message': str(e)}
    )

  except Exception as e:
    logger.error(f'Error getting preferences: {str(e)}', exc_info=True, user_id=user_id)
    raise HTTPException(
      status_code=503,
      detail={
...
async def save_preference(
  http_request: Request, request: SavePreferenceRequest, user_id: str = Depends(get_current_user_id)
):
  ...
  """
  logger.info(
    'Saving user preference', user_id=user_id, preference_key=request.preference_key.value
  )
  ...
  try:
    ...
    # Other ValueError
    logger.warning(f'Invalid preference data: {str(e)}', user_id=user_id)
    raise HTTPException(
      status_code=400, detail={'error_code': 'INVALID_PREFERENCE', 'message': str(e)}
    )

  except Exception as e:
    logger.error(f'Error saving preference: {str(e)}', exc_info=True, user_id=user_id)
    raise HTTPException(
      status_code=503,
      detail={
        'error_code': 'DATABASE_UNAVAILABLE',
...
async def delete_preference(
  request: Request, preference_key: str, user_id: str = Depends(get_current_user_id)
):
  ...
  Raises:
      ...
      503: Database unavailable (EC-002)
  """
  logger.info('Deleting user preference', user_id=user_id, preference_key=preference_key)

  try:
    # LakebaseService uses service principal for database connection (FR-011)
    service = LakebaseService()
    deleted = await service.delete_preference(user_id=user_id, preference_key=preference_key)

    if not deleted:
      logger.warning(
        'Preference not found for deletion', user_id=user_id, preference_key=preference_key
      )
      raise HTTPException(
        status_code=404,
        detail={
    ...
    logger.info(
      'Deleted user preference successfully', user_id=user_id, preference_key=preference_key
    )
    ...
    logger.error(f'Error deleting preference: {str(e)}', exc_info=True, user_id=user_id)
    raise HTTPException(
      status_code=503,
      detail={
        'error_code': 'DATABASE_UNAVAILABLE',
