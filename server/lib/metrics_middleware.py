...
async def metrics_collection_middleware(request: Request, call_next):
  ...
  if not is_lakebase_configured():
    logger.debug(
      f'Skipping performance metric collection for {method} {endpoint} - Lakebase not configured'
    )
    return response
  ...
  try:
    ...
    if any(
      ...
      logger.debug(f'Skipping metrics collection - database unavailable: {type(e).__name__}')
    else:
