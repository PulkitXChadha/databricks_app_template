...
class UserIdentity(BaseModel):
  ...
  extracted_at: datetime = Field(
    default_factory=datetime.utcnow, description='When identity was extracted'
  )
...
class WorkspaceInfoResponse(BaseModel):
  ...
  model_config = {
    'json_schema_extra': {
      'example': {
        'user': {'userName': 'user@example.com', 'displayName': 'John Doe', 'active': True},
        'workspace': {
...
class AuthenticationStatusResponse(BaseModel):
  ...
  auth_mode: str = Field(
    ..., description="Authentication mode (always 'obo' - OBO-only authentication)"
  )
...
class AuthenticationErrorResponse(BaseModel):
  ...
  retry_after: Optional[int] = Field(
    None, description='Seconds to wait before retry (for rate limiting)'
  )
