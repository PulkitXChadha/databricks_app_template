...
class LakebaseService:
  ...
  async def get_preferences(
    self, user_id: str, preference_key: str | None = None
  ) -> list[dict[str, Any]]:
  ...
  def _query_preferences(
    self, session: Session, user_id: str, preference_key: str | None
  ) -> list[dict[str, Any]]:
  ...
  async def save_preference(
    self, user_id: str, preference_key: str, preference_value: dict[str, Any]
  ) -> dict[str, Any]:
    ...
    if not validate_preference_key(preference_key):
      raise ValueError(
        f'Invalid preference_key: {preference_key}. Allowed keys: dashboard_layout, favorite_tables, theme'
      )
  ...
  def _save_preference(
    self, session: Session, user_id: str, preference_key: str, preference_value: dict[str, Any]
  ) -> dict[str, Any]:
    ...
    existing = (
      ...
      .filter_by(user_id=user_id, preference_key=preference_key)
      .first()
    )
  ...
  def _delete_preference(self, session: Session, user_id: str, preference_key: str) -> bool:
  ...
  async def get_preference(self, user_id: str, key: str) -> dict[str, Any] | None:
