...
class UnityCatalogService:
  """Service for Unity Catalog data access with OBO-only authentication.
  ...
  def __init__(self, user_token: str | None = None):
    ...
    if user_token:
      ...
      self.client = WorkspaceClient(host=databricks_host)
  ...
  async def list_catalogs(self, user_id: str | None = None) -> list[str]:
    ...
    try:
      ...
      logger.info(f'Listed {len(catalogs)} catalogs', user_id=user_id)

      return sorted(catalogs)

    except DatabricksError as e:
      logger.error(f'Unity Catalog error: {str(e)}', exc_info=True, user_id=user_id)
      raise

  async def list_schemas(self, catalog: str, user_id: str | None = None) -> list[str]:
    ...
    try:
      ...
      logger.info(
        f'Listed {len(schemas)} schemas in catalog {catalog}', user_id=user_id, catalog=catalog
      )
      ...
      logger.error(
        f'Unity Catalog error: {str(e)}', exc_info=True, user_id=user_id, catalog=catalog
      )
  ...
  async def list_table_names(
    self, catalog: str, schema: str, user_id: str | None = None
  ) -> list[str]:
    ...
    try:
      ...
      table_list = self.client.tables.list(catalog_name=catalog, schema_name=schema)
  ...
  async def list_tables(
    self, catalog: str | None = None, schema: str | None = None, user_id: str | None = None
  ) -> list[DataSource]:
    ...
    try:
      ...
      # List tables in catalog.schema
      table_list = self.client.tables.list(catalog_name=catalog_name, schema_name=schema_name)
  ...
  async def _execute_count_query(self, catalog: str, schema: str, table: str) -> int | None:
    ...
    try:
      count_response = self.client.statement_execution.execute_statement(
        warehouse_id=self.warehouse_id, statement=count_statement, wait_timeout='30s'
      )

      if (
        hasattr(count_response, 'status')
        and count_response.status
        and count_response.status.state == StatementState.SUCCEEDED
        and hasattr(count_response, 'result')
        and count_response.result
        and hasattr(count_response.result, 'data_array')
        and count_response.result.data_array
      ):
  ...
  def _remap_column_names(
    self, rows: list[dict[str, Any]], data_source: DataSource, result_column_names: list[str]
  ) -> list[dict[str, Any]]:
  ...
  async def query_table(
    ...
    try:
      ...
      response = self.client.statement_execution.execute_statement(
        warehouse_id=self.warehouse_id, statement=sql_statement, wait_timeout='30s'
      )
      ...
      if response.status.state == StatementState.SUCCEEDED:
        ...
        data_source = await self._get_table_metadata(
          catalog, schema, table, fallback_columns=columns
        )
  ...
  def _convert_rows_to_dicts(
    self, data_array: list[list[Any]], column_names: list[str]
  ) -> list[dict[str, Any]]:
  ...
  def _parse_result_data(self, result: Any) -> tuple[list[dict[str, Any]], list[str]]:
    ...
    try:
      ...
      logger.info(
        f'Parsed {len(rows)} rows with {len(column_names) if column_names else "generic"} column names'
      )
    ...
    try:
      ...
      return DataSource(
        ...
        if columns
          ...
          fallback_columns or [ColumnDefinition(name='unknown', data_type='STRING', nullable=True)]
        ),
        owner=table_info.owner or 'unknown',
      ...
      return DataSource(
        ...
        or [ColumnDefinition(name='unknown', data_type='STRING', nullable=True)],
        owner='unknown',
        access_level=AccessLevel.READ,
        last_refreshed=datetime.utcnow(),
      )
