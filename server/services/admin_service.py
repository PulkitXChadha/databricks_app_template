...
def _get_admin_group_names() -> set[str]:
    ...
    """
    admin_groups_env = os.getenv(
        'ADMIN_GROUPS',
        'admins,workspace_admins,administrators'
    )
...
async def is_workspace_admin(user_token: str, user_id: str) -> bool:
    ...
    if cache_key in _admin_cache:
        ...
        if datetime.utcnow() < cached_result['expires_at']:
            logger.debug(
                f"Admin check cache hit for user {user_id}: {cached_result['is_admin']}"
            )
    ...
    try:
        ...
        is_admin = any(
            group.display.lower() in admin_group_names
