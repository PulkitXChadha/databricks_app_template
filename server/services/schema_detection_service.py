...
class SchemaDetectionService:
  ...
  async def detect_schema(self, endpoint_name: str, user_id: str) -> SchemaDetectionResult:
    ...
    try:
      ...
      # Step 4: MLflow model path (T023)
        ...
        try:
          mlflow_schema = await asyncio.wait_for(
            self.retrieve_mlflow_schema(model_name, model_version), timeout=5.0
          )
      ...
      if (
        '403' in error_msg or 'permission' in error_msg.lower() or 'forbidden' in error_msg.lower()
      ):
        error_message = (
          f"Permission Denied: {error_msg}. You may not have access to this endpoint's schema."
        )
  ...
  def detect_endpoint_type(self, endpoint: ModelEndpoint) -> EndpointType:
    ...
    if 'served_models' in endpoint.config and endpoint.config['served_models']:
      ...
      if (
        isinstance(served_model, dict)
        and 'model_name' in served_model
        and served_model['model_name']
      ):
  ...
  async def retrieve_mlflow_schema(self, model_name: str, version: str) -> dict | None:
    ...
    for attempt in range(max_retries + 1):
      try:
        ...
        model_version = await asyncio.wait_for(
          asyncio.to_thread(
            self.client.model_registry.get_model_version, full_name=model_name, version=version
          ),
          timeout=5.0,
        )
        ...
        if not hasattr(model_version, 'signature') or not model_version.signature:
          logger.warning('MLflow model has no signature', model_name=model_name, version=version)
          return None
  ...
  async def log_detection_event(self, event: SchemaDetectionEvent) -> None:
