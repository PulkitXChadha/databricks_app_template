...
async def test_endpoint_formats(endpoint_name: str):
  ...
  test_cases = [
    ...
    {'name': 'Traditional ML Model - Inputs Wrapper', 'body': {'inputs': [[1.0, 2.0, 3.0, 4.0]]}},
    {
      'name': 'Traditional ML Model - Dataframe Split',
      'body': {'dataframe_split': {'columns': ['feature1', 'feature2'], 'data': [[1.0, 2.0]]}},
    },
    {'name': 'Custom - Input Parameter', 'body': {'input': 'Hello, how are you?'}},
  ]
  ...
  async with httpx.AsyncClient(timeout=30.0) as http_client:
    for i, test_case in enumerate(test_cases, 1):
      ...
      try:
        response = await http_client.post(url, json=test_case['body'], headers=headers)
