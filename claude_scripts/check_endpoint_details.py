    ...
    if endpoint.config:
      ...
      if endpoint.config.served_models:
        ...
        for i, model in enumerate(endpoint.config.served_models, 1):
          ...
          print(
            f'    Model Name (Registry): {model.model_name if hasattr(model, "model_name") else "N/A"}'
          )
          print(
            f'    Model Version: {model.model_version if hasattr(model, "model_version") else "N/A"}'
          )
          print(f'    Workload Size: {model.workload_size}')
          print(
            f'    Workload Type: {model.workload_type if hasattr(model, "workload_type") else "N/A"}'
          )
          ...
          if hasattr(model, 'external_model'):
            ...
            print('    External Model: NO (Custom/MLflow model)')
