...
def get_warehouses():
  ...
  returncode, stdout, stderr = run_command('databricks warehouses list --output json')
...
def main():
  parser = argparse.ArgumentParser(description='Retrieve Lakebase connection details')
  parser.add_argument(
    '--instance-name', help='Lakebase instance name (auto-detected from TARGET if not provided)'
  )
  parser.add_argument(
    '--warehouse-name', help='SQL Warehouse name (auto-detected from TARGET if not provided)'
  )
  parser.add_argument(
    '--target',
  ...
  if args.instance_name:
    ...
    instance_name = (
      f'databricks-app-lakebase-{args.target}'
      if args.target == 'dev'
      else 'databricks-app-lakebase'
    )
  ...
  if args.warehouse_name:
    ...
    warehouse_name = (
      f'databricks-app-warehouse-{args.target}'
      if args.target == 'dev'
      else 'databricks-app-warehouse'
    )
  ...
  if args.output_format == 'json':
    ...
    if lakebase_host:
      ...
      print('LAKEBASE_DATABASE=app_database')
