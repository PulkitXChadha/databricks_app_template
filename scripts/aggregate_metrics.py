...
def aggregate_performance_metrics(session: Session, cutoff_date: datetime) -> int:
    ...
    for (hour_bucket, endpoint), metrics_group in hourly_buckets.items():
        ...
        existing = session.query(AggregatedMetric).filter(
            and_(
    ...
    delete_count = session.query(PerformanceMetric).filter(
        PerformanceMetric.timestamp < cutoff_date
    ).delete()
    
    logger.info(
        f"Aggregated {aggregated_count} performance metric buckets, "
        f"deleted {delete_count} raw records"
    )
...
def aggregate_usage_events(session: Session, cutoff_date: datetime) -> int:
    ...
    delete_count = session.query(UsageEvent).filter(
        UsageEvent.timestamp < cutoff_date
    ).delete()
    
    logger.info(
        f"Aggregated {aggregated_count} usage event buckets, "
        f"deleted {delete_count} raw records"
    )
...
def cleanup_old_aggregated_metrics(session: Session) -> int:
    ...
    delete_count = session.query(AggregatedMetric).filter(
        AggregatedMetric.time_bucket < cutoff_date
    ).delete()
