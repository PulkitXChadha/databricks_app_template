...
def format_json_log(log_dict: Dict[str, Any]) -> str:
  ...
  if 'message' in log_dict:
    ...
    message = re.sub(
      r'\b(GET|POST|PUT|DELETE|PATCH|OPTIONS)\b',
      f'{Colors.BOLD}{Colors.BRIGHT_MAGENTA}\\1{Colors.RESET}',
      message,
    )
    message = re.sub(r'\b(2\d{2})\b', f'{Colors.BRIGHT_GREEN}\\1{Colors.RESET}', message)
    message = re.sub(r'\b(4\d{2})\b', f'{Colors.BRIGHT_YELLOW}\\1{Colors.RESET}', message)
    message = re.sub(r'\b(5\d{2})\b', f'{Colors.BRIGHT_RED}\\1{Colors.RESET}', message)
  ...
  for field in important_fields:
    if field in log_dict and field not in skip_fields:
      ...
      if field == 'duration_ms':
        ...
        context.append(
          f'{Colors.DIM}{field}:{Colors.RESET} {Colors.BRIGHT_WHITE}{value}{Colors.RESET}'
        )
...
def format_uvicorn_log(line: str) -> str:
  ...
  match = re.match(
    r'^(INFO|WARNING|ERROR):\s+(.+?)\s+-\s+"(\w+)\s+(.+?)\s+HTTP/[\d.]+"?\s+(\d+)\s+(.+)$', line
  )
  if match:
...
def format_vite_log(line: str) -> str:
  ...
  # Highlight URLs
  line = re.sub(r'(https?://[^\s]+)', f'{Colors.BRIGHT_CYAN}\\1{Colors.RESET}', line)

  # Highlight "ready" messages
  if 'ready' in line.lower():
    line = re.sub(r'(ready)', f'{Colors.BRIGHT_GREEN}\\1{Colors.RESET}', line, flags=re.IGNORECASE)
