...
def upgrade():
    ...
    existing_constraints = [con['name'] for con in inspector.get_unique_constraints('user_preferences')]
    ...
    if 'uq_user_preference' not in existing_constraints:
        op.create_unique_constraint('uq_user_preference', 'user_preferences',
    ...
    if 'user_id' not in existing_columns_logs:
        ...
        op.add_column('model_inference_logs',
            sa.Column('user_id', sa.String(255), nullable=False,
                      server_default='migration-placeholder@example.com')
        )
