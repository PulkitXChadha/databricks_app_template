...
class LogzClient:
  ...
  def fetch_logs(
    self, search_query: str = '', watch: bool = False, interval: int = 5
  ) -> List[Dict[str, Any]]:
    ...
    try:
      ...
      if self.client.is_local:
        ...
        print('')
        print(
          '   For local development, check logs directly in your terminal where the app is running.'
        )
      else:
        print(f'❌ Error fetching logs: {e}')
      return []
...
Examples:
  ...
  parser.add_argument(
    '--app_url',
    help='Base URL of the Databricks app. For local: http://localhost:8000 (FastAPI) or auto-detect deployed app if not provided)',
  )
  ...
  parser.add_argument(
    '--interval', type=int, default=5, help='Interval between fetches when streaming (seconds)'
  )
