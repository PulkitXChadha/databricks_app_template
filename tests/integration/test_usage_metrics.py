...
def test_page_view_event_recorded(test_db_session):
  ...
  with patch('server.lib.auth.get_user_token', return_value='mock-token'):
    with patch('databricks.sdk.WorkspaceClient') as mock_client:
      ...
      stored_events = (
        test_db_session.query(UsageEvent)
        .filter(UsageEvent.event_type == 'page_view', UsageEvent.page_name == '/metrics')
        .all()
      )
  ...
  with patch('server.lib.auth.get_user_token', return_value='mock-token'):
    with patch('databricks.sdk.WorkspaceClient') as mock_client:
      ...
      stored_count = (
        test_db_session.query(UsageEvent)
        .filter(
          UsageEvent.user_id == 'batch-test@example.com', UsageEvent.event_type == 'button_click'
        )
        .count()
      )
...
def test_7_day_old_usage_events_aggregated(test_db_session):
  ...
  try:
    ...
    aggregated = (
      ...
      .filter(
        AggregatedMetric.metric_type == 'usage', AggregatedMetric.event_type == 'query_executed'
      )
      .first()
    )
    ...
    remaining_events = (
      test_db_session.query(UsageEvent).filter(UsageEvent.timestamp < cutoff_date).count()
    )
...
def test_usage_events_with_metadata(test_db_session):
  ...
  with patch('server.lib.auth.get_user_token', return_value='mock-token'):
    with patch('databricks.sdk.WorkspaceClient') as mock_client:
      ...
      stored_event = (
        test_db_session.query(UsageEvent)
        .filter(UsageEvent.event_type == 'query_executed', UsageEvent.user_id == 'test@example.com')
        .first()
      )
