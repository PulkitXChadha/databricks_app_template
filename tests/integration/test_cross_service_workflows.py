...
from server.models.model_inference import InferenceStatus, ModelInferenceResponse
...
def mock_all_services_for_workflow():
  ...
  unity_service_instance.query_table = AsyncMock(
    return_value={
      'rows': [
  ...
  unity_mock_patch = patch(
    'server.routers.unity_catalog.UnityCatalogService', return_value=unity_service_instance
  )
  unity_mock = unity_mock_patch.__enter__()

  # Mock Model Serving Service
  model_service_instance = Mock()
  model_service_instance.list_endpoints = AsyncMock(
    return_value=[
      {
  ...
  model_service_instance.get_endpoint = AsyncMock(
    return_value={
      'name': 'claude-sonnet-4',
      'endpoint_type': 'FOUNDATION_MODEL_API',
      'state': {'ready': 'READY'},
  ...
  model_service_instance.invoke_model = AsyncMock(
    return_value=ModelInferenceResponse(
  ...
  model_mock_patch = patch(
    'server.routers.model_serving.ModelServingService', return_value=model_service_instance
  )
  ...
  lakebase_service_instance.save_preference = AsyncMock(
    return_value={
      'id': 1,
  ...
  lakebase_mock_patch = patch(
    'server.routers.lakebase.LakebaseService', return_value=lakebase_service_instance
  )
...
@pytest.mark.skip(
  reason='Cross-service workflow tests require additional mocking complexity - see tasks.md T068-T070'
)
class TestCrossServiceWorkflows:
  ...
  def test_end_to_end_catalog_to_inference_workflow(self, client, test_user_a):
    ...
    with mock_user_context(user_id):
      with mock_all_services_for_workflow() as (unity_service, model_service, lakebase_service):
        ...
        catalogs_response = client.get('/api/unity-catalog/catalogs', headers=headers)
        ...
        catalog_name = (
          catalogs[0] if isinstance(catalogs[0], str) else catalogs[0].get('catalog_name', 'main')
        )
        schemas_response = client.get(
          f'/api/unity-catalog/schemas?catalog={catalog_name}', headers=headers
        )
        ...
        endpoints_response = client.get('/api/model-serving/endpoints', headers=headers)
        ...
        invoke_payload = {
          ...
          'inputs': {
            'messages': [{'role': 'user', 'content': f'Analyze this data: {query_data["rows"][0]}'}]
          },
        }

        invoke_response = client.post(
          '/api/model-serving/invoke', json=invoke_payload, headers=headers
        )
  ...
  def test_preferences_customize_catalog_queries(self, client, test_user_a):
    ...
    with mock_user_context(user_id):
      with mock_all_services_for_workflow() as (unity_service, model_service, lakebase_service):
        ...
        pref_response = client.post('/api/preferences', json=preference_payload, headers=headers)
        ...
        lakebase_service.get_preferences = AsyncMock(
          return_value=[
            {
              'id': 1,
              'user_id': user_id,
        ...
        get_pref_response = client.get('/api/preferences', headers=headers)
        ...
        favorite_tables_pref = next(
          (p for p in preferences if p['preference_key'] == 'favorite_tables'), None
        )
        ...
        assert call_kwargs.get('catalog_name') == 'main' or call_kwargs.get('limit') == 100, (
  ...
  def test_concurrent_users_maintain_isolation(self, client, test_user_a, test_user_b):
      ...
      with mock_user_context(user_id):
        with mock_all_services_for_workflow() as (unity_service, model_service, lakebase_service):
          ...
          pref_payload = {
            ...
            'preference_value': {
              'user_id': user_id,
              'catalog': 'main' if 'user-a' in user_id else 'samples',
            },
          }

          pref_response = client.post('/api/preferences', json=pref_payload, headers=headers)
          ...
          results['query_data'] = (
            query_response.json() if query_response.status_code == 200 else None
          )
  ...
  def test_inference_logs_persist_across_workflow(self, client, test_user_a):
    ...
    with mock_user_context(user_id):
      with mock_all_services_for_workflow() as (unity_service, model_service, lakebase_service):
        ...
        for i in range(inference_count):
          invoke_payload = {
            'endpoint_name': 'claude-sonnet-4',
            'inputs': {'messages': [{'role': 'user', 'content': f'Test inference {i + 1}'}]},
          }
          ...
          logged_inferences.append(
            {'endpoint_name': 'claude-sonnet-4', 'status': 'SUCCESS', 'user_id': user_id}
          )
        ...
        logs_response = client.get('/api/model-serving/logs?limit=10&offset=0', headers=headers)
...
def create_mock_catalog_response(catalog_name: str) -> Dict[str, Any]:
  ...
  return {'catalog_name': catalog_name, 'owner': 'admin', 'comment': f'Test catalog {catalog_name}'}
