...
class TestModelInputValidation:
  ...
  def valid_sentiment_input(self):
    ...
    return {
      ...
      'inputs': {'text': 'This product is amazing! Highly recommend.'},
      'timeout_seconds': 30,
    }
  ...
  def test_invalid_json_syntax(self, client, mock_user):
    ...
    with patch('server.routers.model_serving.get_current_user_id', return_value=mock_user):
      ...
      assert response.status_code == 422, 'Malformed JSON should return 422 Unprocessable Entity'
  ...
  def test_type_mismatch(self, client, mock_user):
    ...
    with patch('server.routers.model_serving.get_current_user_id', return_value=mock_user):
      ...
      assert response.status_code == 422, 'Type mismatch should return 422'
      ...
      assert any(
        keyword in error_str.lower() for keyword in ['type', 'int', 'integer', 'number']
  ...
  def test_constraint_violation(self, client, mock_user):
    ...
    with patch('server.routers.model_serving.get_current_user_id', return_value=mock_user):
      ...
      assert response.status_code == 422, 'Constraint violation should return 422'
  ...
  def test_server_side_rejection(self, mock_invoke, client, mock_user, valid_sentiment_input):
    ...
    with patch('server.routers.model_serving.get_current_user_id', return_value=mock_user):
      ...
      response = client.post('/api/model-serving/invoke', json=valid_sentiment_input)

      # Application should return error response
      assert response.status_code in [400, 500, 503], 'Model rejection should return error status'
  ...
  def test_invalid_endpoint_name(self, mock_list_endpoints, client, mock_user):
    ...
    with patch('server.routers.model_serving.get_current_user_id', return_value=mock_user):
      ...
      assert response.status_code in [404, 500, 503], 'Invalid endpoint should return error status'
  ...
  def test_timeout_boundary_values(self, client, mock_user):
    ...
    with patch('server.routers.model_serving.get_current_user_id', return_value=mock_user):
      with patch(
        'server.services.model_serving_service.ModelServingService.invoke_model', return_value={}
      ):
        # Test minimum valid value (1)
        response_min = client.post(
          '/api/model-serving/invoke',
          json={'endpoint_name': 'test-endpoint', 'inputs': {'text': 'test'}, 'timeout_seconds': 1},
        )
        ...
        response_too_low = client.post(
          '/api/model-serving/invoke',
          json={'endpoint_name': 'test-endpoint', 'inputs': {'text': 'test'}, 'timeout_seconds': 0},
        )
        assert response_too_low.status_code == 422, 'Timeout below minimum (0) should be rejected'
