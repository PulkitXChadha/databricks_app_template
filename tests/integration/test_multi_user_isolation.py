...
class TestMultiUserIsolation:
    ...
    try:
      ...
      session.query(UserPreference).filter(
        UserPreference.user_id.in_(
          ['user-a@company.com', 'user-b@company.com', 'dev-user@example.com']
        )
  ...
  def test_lakebase_preference_isolation(self, client, mock_user_a, mock_user_b):
    ...
    with patch('server.routers.lakebase.get_current_user_id') as mock_get_user:
      ...
      response_a_create = client.post(
        '/api/preferences', json={'preference_key': 'theme', 'preference_value': {'mode': 'dark'}}
      )
      ...
      response_b_create = client.post(
        '/api/preferences', json={'preference_key': 'theme', 'preference_value': {'mode': 'light'}}
      )
  ...
  def test_unity_catalog_permission_isolation(
    self, mock_list_tables, client, mock_user_a, mock_user_b
  ):
    ...
    with patch('server.routers.lakebase.get_current_user_id') as mock_get_user:
      ...
      client.post(
        ...
        json={
          ...
          'preference_value': {'tables': ['table_a', 'table_c']},
        },
      )
      ...
      client.post(
        ...
        json={
          'preference_key': 'favorite_tables',
          'preference_value': {'tables': ['table_b', 'table_d']},
        },
      )
