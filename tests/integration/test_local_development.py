  ...
  def test_api_requests_succeed_with_cli_token(self, client, mock_cli_token):
    ...
    with patch('server.services.user_service.WorkspaceClient') as mock_client:
      ...
      mock_user = type(
        'User',
        (),
        {'user_name': 'developer@example.com', 'display_name': 'Local Developer', 'active': True},
      )()

      mock_ws_client = type(
        'WSClient', (), {'current_user': type('CurrentUser', (), {'me': lambda: mock_user})()}
      )()

      mock_client.return_value = mock_ws_client

      # Make request with CLI token
      response = client.get('/api/user/me', headers={'X-Forwarded-Access-Token': mock_cli_token})

      # Should succeed with user-level authentication
      assert response.status_code == 200, 'API request should succeed with CLI token'
...
class TestLocalDevelopmentErrorHandling:
  ...
  def test_invalid_token_returns_clear_error(self, client):
    ...
    with patch('server.services.user_service.WorkspaceClient') as mock_client:
      ...
      mock_ws_client = type(
        'WSClient', (), {'current_user': type('CurrentUser', (), {'me': raise_error})()}
      )()

      mock_client.return_value = mock_ws_client

      response = client.get('/api/user/me', headers={'X-Forwarded-Access-Token': 'invalid-token'})
      ...
      assert error_data['error_code'] in [
        'AUTH_INVALID',
        'AUTH_EXPIRED',
...
class TestLocalDevelopmentDocumentation:
  ...
  def test_local_development_docs_exist(self):
    ...
    docs_path = os.path.join(os.path.dirname(__file__), '../../docs/LOCAL_DEVELOPMENT.md')
  ...
  def test_get_user_token_script_exists(self):
    ...
    script_path = os.path.join(os.path.dirname(__file__), '../../scripts/get_user_token.py')
