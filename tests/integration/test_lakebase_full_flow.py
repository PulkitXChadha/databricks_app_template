...
class TestLakebaseFullFlow:
  ...
  def test_get_preferences_empty_when_no_data(self, client, test_user_a, mock_user_auth):
    ...
    with mock_user_context(test_user_a['user_id']):
      # Act: GET preferences
      response = client.get(
        '/api/preferences', headers={'X-Forwarded-Access-Token': test_user_a['token']}
      )
  ...
  def test_preference_isolation_between_users(self, app, test_user_a, test_user_b):
    ...
    try:
      ...
      with patch('server.lib.database.is_lakebase_configured') as mock_is_configured:
        ...
        assert response_b.status_code == 201, (
          f'Setup failed: {response_b.status_code}, Response: {response_b.json()}'
        )
  ...
  def test_upsert_behavior_on_duplicate_key(self, client, test_user_a, mock_user_auth):
    ...
    initial_data = {'preference_key': 'theme', 'preference_value': {'mode': 'light'}}
    ...
    with mock_user_context(test_user_a['user_id']):
      ...
      assert theme_pref is not None, 'Theme preference not found after upsert'
  ...
  def test_delete_nonexistent_preference_returns_404(self, client, test_user_a, mock_user_auth):
    ...
    with mock_user_context(test_user_a['user_id']):
      ...
      data = response.json()
      assert 'detail' in data, f"Expected 'detail' field in error response, got {data.keys()}"
  ...
  def test_large_preference_value_rejected(self, client, test_user_a, mock_user_auth):
    ...
    large_data = {'preference_key': 'dashboard_layout', 'preference_value': large_value}
  ...
  def test_concurrent_preference_update_last_write_wins(self, client, test_user_a, mock_user_auth):
    ...
    with mock_user_context(test_user_a['user_id']):
      for value_data in values_to_write:
        response = client.post(
          '/api/preferences',
          json={'preference_key': 'dashboard_layout', 'preference_value': value_data},
          headers={'X-Forwarded-Access-Token': test_user_a['token']},
        )
      ...
      assert dashboard_pref is not None, 'Expected to find dashboard_layout preference after writes'

      # Verify: Final value is the last written value (version 3)
      final_value = dashboard_pref['preference_value']
      assert 'version' in final_value, f'Expected version field in final value, got {final_value}'
  ...
  def test_special_characters_unicode_in_preference_keys(self, client, test_user_a, mock_user_auth):
    ...
    test_data = {'preference_key': 'dashboard_layout', 'preference_value': special_value}
    ...
    with mock_user_context(test_user_a['user_id']):
      ...
      data = get_response.json()
      dashboard_pref = next((p for p in data if p['preference_key'] == 'dashboard_layout'), None)
      assert dashboard_pref is not None, 'Expected to find dashboard_layout preference'
      ...
      assert retrieved_value['mixed'] == special_value['mixed'], 'Mixed content not preserved'
