...
class TestPaginationPerformance:
  ...
  def _measure_request_time(
    self, client, endpoint: str, params: dict = None, json_data: dict = None
  ):
  ...
  def test_single_user_baseline_performance(self, client, mock_user, mock_query_result):
    ...
    with patch('server.routers.unity_catalog.get_current_user_id', return_value=mock_user):
      with patch(
        'server.services.unity_catalog_service.UnityCatalogService.query_table',
        ...
        for i, latency in enumerate(latencies):
          assert latency < 500, f'Request {i + 1} exceeded 500ms target: {latency:.2f}ms'
    ...
    with patch('server.routers.unity_catalog.get_current_user_id', return_value=mock_user):
      ...
      for page_size in page_sizes:
        ...
        with patch(
          ...
          if page_size <= 100:
            assert duration_ms < 500, f'Page size {page_size} exceeded 500ms: {duration_ms:.2f}ms'
          else:  # page_size == 500
            assert duration_ms < 1000, f'Page size {page_size} exceeded 1000ms: {duration_ms:.2f}ms'
  ...
  def test_model_inference_performance(self, mock_invoke, client, mock_user):
    ...
    with patch('server.routers.model_serving.get_current_user_id', return_value=mock_user):
      ...
      for i in range(5):
        ...
        assert duration_ms < 2000, f'Inference request {i + 1} exceeded 2000ms: {duration_ms:.2f}ms'
