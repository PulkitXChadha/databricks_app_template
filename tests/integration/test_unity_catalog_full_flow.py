...
def assert_error_response(
    ...
    if error_keywords:
        ...
        assert found_keyword, \
            f"Expected one of {error_keywords} in response, got {response_data}"
...
class TestUnityCatalogFullFlow:
    ...
    def test_get_catalogs_returns_accessible_catalogs(
        ...
        with mock_user_context(test_user_a["user_id"]):
            ...
            with mock_unity_catalog_service('list_catalogs', return_value=mock_catalog_metadata["catalogs"]):
                ...
                assert isinstance(catalogs, list), \
                    f"Expected list of catalogs, got {type(catalogs)}"
                assert len(catalogs) > 0, \
                    "Expected at least one catalog, got empty list"
    ...
    def test_get_schemas_for_catalog(
        ...
        with mock_user_context(test_user_a["user_id"]):
            ...
            with mock_unity_catalog_service('list_schemas', return_value=mock_catalog_metadata["schemas"]["main"]):
                ...
                assert isinstance(schemas, list), \
                    f"Expected list of schemas, got {type(schemas)}"
                assert len(schemas) > 0, \
                    "Expected at least one schema, got empty list"
                assert "default" in schemas, \
                    f"Expected 'default' schema in 'main' catalog, got {schemas}"
    ...
    def test_get_table_names_for_schema(
        ...
        with mock_user_context(test_user_a["user_id"]):
            ...
            table_names = [table["table_name"] for table in mock_catalog_metadata["tables"]["main.default"]]
            with mock_unity_catalog_service('list_table_names', return_value=table_names):
                ...
                assert isinstance(tables, list), \
                    f"Expected list of table names, got {type(tables)}"
                assert len(tables) > 0, \
    ...
    def test_get_tables_with_metadata(
        self, client, test_user_a, mock_user_auth
    ):
        ...
        with mock_user_context(test_user_a["user_id"]):
            ...
            with mock_unity_catalog_service('list_tables', return_value=mock_tables):
                ...
                tables = response.json()
                assert isinstance(tables, list), \
                    f"Expected list of tables, got {type(tables)}"
                assert len(tables) > 0, \
                    "Expected at least one table, got empty list"
                
                # Verify DataSource structure
                first_table = tables[0]
                assert "catalog_name" in first_table, \
                    "Expected 'catalog_name' in DataSource"
                assert "schema_name" in first_table, \
                    "Expected 'schema_name' in DataSource"
                assert "table_name" in first_table, \
                    "Expected 'table_name' in DataSource"
                assert "columns" in first_table, \
                    "Expected 'columns' in DataSource"
    
    # ==========================================================================
    # Test 5: Query table with pagination
    # ==========================================================================
    
    def test_query_table_with_pagination(
        self, client, test_user_a, mock_user_auth, mock_table_data
    ):
        ...
        with mock_user_context(test_user_a["user_id"]):
            ...
            with mock_unity_catalog_service('query_table', return_value=mock_result):
                ...
                assert "rows" in result, \
                    "Expected 'rows' field in QueryResult"
                assert "row_count" in result, \
                    "Expected 'row_count' field in QueryResult"
        ...
        with mock_user_context(test_user_a["user_id"]):
            ...
            with mock_unity_catalog_service('query_table', return_value=mock_result):
                ...
                assert "rows" in result, \
                    "Expected 'rows' field in QueryResult"
                assert result["row_count"] > 0, \
                    "Expected at least one row in filtered result"
    
    # ==========================================================================
    # Test 7: Permission denied returns 403
    # ==========================================================================
    
    def test_catalog_permission_denied_returns_403(
        self, client, test_user_a, mock_user_auth
    ):
    ...
    def test_table_not_found_returns_404(
        self, client, test_user_a, mock_user_auth
    ):
        ...
        with mock_user_context(test_user_a["user_id"]):
            ...
            with mock_unity_catalog_service(
                'query_table',
                side_effect=Exception("Table main.default.nonexistent not found")
            ):
    ...
    def test_invalid_pagination_parameters_returns_400(
        self, client, test_user_a, mock_user_auth
    ):
    ...
    def test_pagination_offset_exceeds_total_returns_empty(self, client, mock_user_auth):
        ...
        with mock_unity_catalog_service('query_table', return_value=mock_result):
            with mock_user_context("test-user-a@example.com"):
                ...
                assert "rows" in data, \
                    f"Expected 'rows' field, got {data.keys()}"
                assert isinstance(data["rows"], list), \
                    f"Expected rows to be list, got {type(data['rows'])}"
                assert len(data["rows"]) == 0, \
                    f"Expected empty rows when offset > total, got {len(data['rows'])} rows"
                assert data.get("total_rows", 0) >= 0, \
                    "Expected non-negative total_rows"
