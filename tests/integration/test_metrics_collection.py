    ...
    with patch('server.services.admin_service.is_workspace_admin', return_value=True):
      with patch('server.lib.auth.get_user_token', return_value='mock-token'):
        ...
        metrics = (
          ...
          .filter(PerformanceMetric.endpoint == '/api/v1/metrics/performance')
          .all()
        )
...
def test_middleware_marks_error_metrics_with_error_type(test_db_session):
  ...
  with patch('server.lib.auth.get_user_token', return_value='mock-token'):
    ...
    metrics = (
      test_db_session.query(PerformanceMetric)
      .filter(PerformanceMetric.endpoint == '/api/v1/nonexistent-endpoint')
      .all()
    )
...
def test_middleware_overhead_less_than_5ms(test_db_session):
  ...
  assert avg_time < 10.0, (
    f'Request with metrics collection should be fast, got {avg_time:.2f}ms avg'
  )
...
def test_metrics_service_records_performance_metric(test_db_session):
  ...
  metrics = (
    test_db_session.query(PerformanceMetric)
    .filter(PerformanceMetric.endpoint == '/api/v1/test')
    .all()
  )
...
def test_100_percent_collection_rate(test_db_session):
  ...
  metric_count = (
    test_db_session.query(PerformanceMetric)
    .filter(PerformanceMetric.endpoint.like('/health%'))
    .count()
  )
...
def test_graceful_degradation_on_database_failure(test_db_session):
  ...
  with patch(
    'server.services.metrics_service.MetricsService.record_performance_metric'
  ) as mock_record:
