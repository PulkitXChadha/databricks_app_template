...
def test_7_day_old_metrics_aggregated_and_deleted(test_db_session):
  ...
  aggregated_metrics = (
    test_db_session.query(AggregatedMetric)
    .filter(AggregatedMetric.metric_type == 'performance')
    .all()
  )
  ...
  remaining_old = (
    test_db_session.query(PerformanceMetric)
...
def test_90_day_old_aggregated_metrics_deleted(test_db_session):
  ...
  remaining_very_old = (
    test_db_session.query(AggregatedMetric)
    .filter(AggregatedMetric.time_bucket < now - timedelta(days=90))
    .count()
  )

  assert remaining_very_old == 0, 'Aggregated metrics older than 90 days should be deleted'
  assert deleted_count == len(very_old_aggregated), (
    f'Should delete {len(very_old_aggregated)} records'
  )
...
def test_database_size_monitoring_thresholds_and_alert_prefix(test_db_session, caplog):
  ...
  assert any(
    record.levelname == 'WARNING' and '800' in record.message for record in caplog.records
  ...
  assert len(alert_logs) > 0, "Should log ERROR with 'ALERT:' prefix when exceeding 1M threshold"
