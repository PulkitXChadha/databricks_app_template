...
class TestOBOOnlyEndpoints:
  ...
  def test_user_me_endpoint_returns_401_with_empty_token(self, client):
    ...
    response = client.get('/api/user/me', headers={'X-Forwarded-Access-Token': ''})
...
class TestNoServicePrincipalFallback:
  ...
  def test_no_fallback_to_service_principal_on_missing_token(self, client, caplog):
    ...
    fallback_events = [
      ...
      or (hasattr(record, 'event') and record.event == 'auth.fallback_triggered')
    ]
    assert len(fallback_events) == 0, 'Should not trigger service principal fallback'
    ...
    for record in log_records:
      if hasattr(record, 'mode'):
        ...
        if record.mode:
          assert record.mode == 'obo', "auth.mode should always be 'obo' when present"
  ...
  def test_auth_mode_logs_always_show_obo(self, client, caplog):
    ...
    for event in auth_mode_events:
      if hasattr(event, 'mode'):
        assert event.mode == 'obo', f"auth.mode should be 'obo', got '{event.mode}'"
        assert event.mode != 'service_principal', "auth.mode should never be 'service_principal'"
...
class TestHealthEndpointPublic:
  ...
  def test_health_endpoint_ignores_auth_header(self, client):
    ...
    response = client.get('/health', headers={'X-Forwarded-Access-Token': 'invalid-token'})

    # Health endpoint should ignore authentication headers
    assert response.status_code == 200, '/health should ignore auth headers and remain public'
...
class TestErrorResponseStructure:
  ...
  def test_401_error_has_correlation_id_in_logs(self, client, caplog):
    ...
    assert len(correlation_ids) > 0, 'At least one log event should include correlation_id'
