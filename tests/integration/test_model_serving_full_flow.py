...
@contextmanager
def mock_model_serving_service(
  mock_endpoints=None,
  mock_endpoint=None,
  mock_invoke_response=None,
  mock_logs=None,
  should_raise=None,
):
...
def create_mock_inference_response(
  status: str = 'SUCCESS',
  ...
  def model_dump():
    return {
      ...
      'created_at': mock_response.created_at.isoformat() + 'Z'
      if mock_response.created_at
      else None,
      'completed_at': mock_response.completed_at.isoformat() + 'Z'
      if mock_response.completed_at
      else None,
    }
...
def create_mock_inference_log(
  endpoint_name: str, user_id: str, status: str = 'SUCCESS'
) -> Dict[str, Any]:
...
class TestModelServingFullFlow:
  ...
  def test_list_endpoints_returns_metadata(self, client, test_user_a, mock_user_auth):
    ...
    with mock_user_context(test_user_a['user_id']):
      with mock_model_serving_service(mock_endpoints=mock_endpoints):
        ...
        response = client.get(
          '/api/model-serving/endpoints', headers={'X-Forwarded-Access-Token': test_user_a['token']}
        )
        ...
        assert len(data) == 2, f'Expected 2 endpoints, got {len(data)}'
  ...
  def test_detect_endpoint_schema(self, client, test_user_a, mock_user_auth):
    ...
    mock_schema_result = SchemaDetectionResult(
      ...
      input_schema={
        'type': 'object',
        'properties': {'messages': {'type': 'array'}, 'max_tokens': {'type': 'integer'}},
      },
      example_json={'messages': [{'role': 'user', 'content': 'Hello'}], 'max_tokens': 1000},
      error_message=None,
      latency_ms=50,
  ...
  def test_invoke_model_returns_predictions(self, client, test_user_a, mock_user_auth):
    ...
    inference_request = {
      'endpoint_name': 'claude-sonnet-4',
      'inputs': {'messages': [{'role': 'user', 'content': 'Hello, world!'}], 'max_tokens': 1000},
      'timeout_seconds': 30,
    }
    ...
    with mock_user_context(test_user_a['user_id']):
      with mock_model_serving_service(mock_invoke_response=mock_response):
        ...
        assert 'predictions' in data, f"Expected 'predictions' in response, got keys: {data.keys()}"

  # ==========================================================================
  # Test 6: Inference log persisted to database
  # ==========================================================================

  def test_inference_log_persisted_to_database(
    self, client, test_user_a, mock_user_auth, test_db_session
  ):
    ...
    mock_response = create_mock_inference_response(
      status='SUCCESS', predictions={'choices': [{'text': 'Response'}]}, execution_time_ms=100
    )
    ...
    try:
      ...
      with mock_model_serving_service(mock_logs=(user_b_logs, 1)):
        ...
        response_b = client.get(
          '/api/model-serving/logs', headers={'X-Forwarded-Access-Token': test_user_b['token']}
        )
        ...
        with mock_model_serving_service(mock_logs=([], 0)):
          response_a = client.get(
            '/api/model-serving/logs', headers={'X-Forwarded-Access-Token': test_user_a['token']}
          )

          # Assert: User A should NOT see User B's logs
          assert response_a.status_code == 200, f'Expected 200 OK, got {response_a.status_code}'
  ...
  def test_model_timeout_returns_503(self, client, test_user_a, mock_user_auth):
    ...
    mock_response = create_mock_inference_response(status='TIMEOUT', execution_time_ms=5000)
  ...
  def test_model_inference_input_size_limit(self, client, mock_user_auth):
    ...
    mock_response = create_mock_inference_response(
      predictions=[{'error': 'Input too large'}], status='ERROR'
    )
    ...
    with mock_model_serving_service(mock_invoke_response=mock_response):
      with mock_user_context('test-user-a@example.com'):
        ...
        if response.status_code in [400, 413, 422]:
          data = response.json()
          assert 'detail' in data, f"Expected 'detail' field in validation error, got {data.keys()}"
        else:
  ...
  def test_schema_detection_no_info_available(self, client, mock_user_auth):
    ...
    mock_endpoint = create_mock_endpoint(
      name='unknown-endpoint', model_name='unknown-model', state='READY'
    )
    ...
    with mock_model_serving_service(mock_endpoints=[mock_endpoint], mock_endpoint=mock_endpoint):
      ...
      with patch('server.routers.model_serving.SchemaDetectionService') as MockService:
        ...
        with mock_user_context('test-user-a@example.com'):
          ...
          data = response.json()
          assert 'endpoint_name' in data, f"Expected 'endpoint_name' field, got {data.keys()}"
          assert 'detected_type' in data, f"Expected 'detected_type' field, got {data.keys()}"
