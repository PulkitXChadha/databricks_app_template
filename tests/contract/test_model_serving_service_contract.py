...
class TestModelServingServiceAuthentication:
  ...
  async def test_list_endpoints_respects_user_permissions(self, mock_env):
    ...
    with patch('server.services.model_serving_service.WorkspaceClient') as mock_client_class:
      ...
      assert any(ep.endpoint_name == 'model-endpoint-1' for ep in endpoints_a), (
        'User A should see endpoint 1'
      )
      assert any(ep.endpoint_name == 'model-endpoint-2' for ep in endpoints_a), (
        'User A should see endpoint 2'
      )
      ...
      assert len(endpoints_a) != len(endpoints_b), (
        'Different users should see different endpoint lists'
      )
...
class TestModelServingServiceEndpointOperations:
  ...
  async def test_invoke_model_uses_obo_token(self, mock_env):
    ...
    with (
      patch('server.services.model_serving_service.WorkspaceClient') as mock_client_class,
