...
class TestHealthEndpoint:
  ...
  def test_health_works_with_token(self, client):
    ...
    response = client.get('/health', headers={'X-Forwarded-Access-Token': 'some-token'})
...
class TestMetricsEndpoint:
  ...
  def test_metrics_returns_200_with_valid_token(self, client):
    ...
    try:
      ...
      result = subprocess.run(
        ['databricks', 'auth', 'token'], capture_output=True, text=True, timeout=5
      )

      if result.returncode == 0 and result.stdout.strip():
        token = result.stdout.strip()
        response = client.get('/metrics', headers={'X-Forwarded-Access-Token': token})
        ...
        assert (
          b'auth_requests_total' in response.content
          or b'request_duration_seconds' in response.content
        )
      else:
  ...
  def test_metrics_empty_token_treated_as_missing(self, client):
    ...
    response = client.get('/metrics', headers={'X-Forwarded-Access-Token': ''})
