...
class TestUnityCatalogListTablesContract:
  ...
  def test_list_tables_with_catalog_filter(self):
    ...
    if response.status_code == 200:
      data = response.json()
      for table in data:
        assert table['catalog_name'] == 'main', 'Catalog filter not applied correctly'
...
class TestUnityCatalogQueryContract:
  ...
  def test_query_table_response_structure(self, auth_headers, mock_user_auth):
    ...
    if response.status_code == 200:
      ...
      assert data['row_count'] == len(data['rows']), 'row_count must equal len(rows)'

      # Validate total_row_count if present
      if 'total_row_count' in data and data['total_row_count'] is not None:
        assert isinstance(data['total_row_count'], int), 'total_row_count must be an integer'
        assert data['total_row_count'] >= 0, 'total_row_count must be non-negative'
        assert data['total_row_count'] >= data['row_count'], 'total_row_count must be >= row_count'

      # Validate execution_time_ms is positive
      assert data['execution_time_ms'] > 0, 'execution_time_ms must be positive'
  ...
  def test_query_table_limit_boundaries(self, auth_headers, mock_user_auth):
    ...
    payload = {'catalog': 'main', 'schema': 'samples', 'table': 'demo_data', 'limit': 1}
    response = client.post('/api/unity-catalog/query', json=payload, headers=auth_headers)
    assert response.status_code in [200, 400, 403, 404, 503]
  ...
  def test_query_table_error_response_structure(self):
    ...
    payload = {'catalog': 'main', 'schema': 'samples', 'table': 'nonexistent_table'}
