...
class TestInvalidTokenErrors:
  ...
  def test_user_me_endpoint_invalid_token(self, client):
    ...
    response = client.get(
      '/api/user/me', headers={'X-Forwarded-Access-Token': 'invalid-token-12345'}
    )
  ...
  def test_malformed_token_structure(self, client):
    ...
    for token in malformed_tokens:
      response = client.get('/api/user/me', headers={'X-Forwarded-Access-Token': token})
...
class TestCorrelationIDs:
  ...
  def test_error_response_includes_correlation_id_in_headers(self, client):
    ...
    correlation_header_found = any(
      'correlation' in key.lower() or 'request-id' in key.lower() for key in headers.keys()
    )
