...
class TestAuthenticationMiddleware:
  ...
  def test_set_has_user_token_correctly(self):
    ...
    response_with = client.get(
      '/api/user/auth/status', headers={'X-Forwarded-Access-Token': 'token'}
    )
  ...
  def test_never_log_token_value(self):
    ...
    response = client.get(
      '/api/user/auth/status', headers={'X-Forwarded-Access-Token': sensitive_token}
    )
