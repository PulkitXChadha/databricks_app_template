...
class TestPreferencesContract:
  ...
  def test_get_preferences_returns_only_authenticated_users_preferences(
    ...
    with patch(
      'server.services.lakebase_service.LakebaseService.get_preferences', new_callable=AsyncMock
    ) as mock_get_prefs:
      ...
      response = client.get('/api/preferences', headers={'X-Forwarded-Access-Token': user_token})
  ...
  def test_post_preference_saves_with_correct_user_id(self, client, mock_user_auth):
    ...
    with patch(
      'server.services.lakebase_service.LakebaseService.save_preference', new_callable=AsyncMock
    ) as mock_save_pref:
      ...
      mock_save_pref.assert_called_once_with(
        user_id=user_id, preference_key='theme', preference_value={'mode': 'dark'}
      )

  def test_delete_preference_only_deletes_authenticated_users_preference(
    self, client, mock_user_auth
  ):
    ...
    with patch(
      'server.services.lakebase_service.LakebaseService.delete_preference', new_callable=AsyncMock
    ) as mock_delete_pref:
      mock_delete_pref.return_value = True  # Successfully deleted

      response = client.delete(
        '/api/preferences/theme', headers={'X-Forwarded-Access-Token': user_token}
      )
  ...
  def test_cross_user_access_prevented(self, client, mock_user_auth):
    ...
    with patch(
      ...
      response_a = client.get('/api/preferences', headers={'X-Forwarded-Access-Token': user_token})
      ...
      response_b = client.get('/api/preferences', headers={'X-Forwarded-Access-Token': user_token})
