...
def test_get_performance_metrics_endpoint_returns_200_for_admin(
  mock_admin_check_true, mock_databricks_admin_client, mock_db_for_metrics
):
...
def test_get_performance_metrics_accepts_time_range_param(
  mock_admin_check_true, mock_databricks_admin_client, mock_db_for_metrics
):
...
def test_get_performance_metrics_accepts_endpoint_filter(
  mock_admin_check_true, mock_databricks_admin_client, mock_db_for_metrics
):
...
def test_get_performance_metrics_returns_403_for_non_admin(
  mock_admin_check_false, mock_databricks_non_admin_client
):
  ...
  """
  response = client.get(
    '/api/v1/metrics/performance', headers={'X-Forwarded-Access-Token': 'mock-non-admin-token'}
  )
...
def test_post_usage_events_accepts_batch():
  ...
  with patch('server.lib.auth.get_user_token', return_value='mock-token'):
    with patch('databricks.sdk.WorkspaceClient') as mock_client:
      ...
      events = [
        {'event_type': 'page_view', 'page_name': '/metrics', 'timestamp': '2025-10-20T12:00:00Z'},
        {
...
def test_get_usage_metrics_endpoint_exists(
  ...
  """
  response = client.get(
    '/api/v1/metrics/usage', headers={'X-Forwarded-Access-Token': 'mock-admin-token'}
  )

  assert response.status_code == 200, 'Endpoint should return 200 for admin'


def test_get_usage_metrics_returns_403_for_non_admin(
  mock_admin_check_false, mock_databricks_non_admin_client, mock_db_for_metrics
):
  """Test that non-admin users receive 403 for usage metrics"""
  response = client.get('/api/v1/metrics/usage', headers={'X-Forwarded-Access-Token': 'mock-token'})
...
def test_post_usage_events_rejects_oversized_batch():
  ...
  with patch('server.lib.auth.get_user_token', return_value='mock-token'):
    ...
    oversized_batch = [
      {'event_type': 'page_view', 'page_name': '/test', 'timestamp': f'2025-10-20T12:{i:02d}:00Z'}
      for i in range(1001)
    ]
    ...
    assert response.status_code == 413, (
      f'Expected 413 for oversized batch, got {response.status_code}'
    )
...
def test_get_time_series_endpoint_returns_200_for_admin(
  mock_admin_check_true, mock_databricks_admin_client, mock_db_for_metrics
):
...
def test_get_time_series_requires_metric_type_parameter(
  mock_admin_check_true, mock_databricks_admin_client, mock_db_for_metrics
):
...
def test_get_time_series_accepts_metric_type_values(
  mock_admin_check_true, mock_databricks_admin_client, mock_db_for_metrics
):
...
def test_get_time_series_returns_5min_interval_for_24h(
  mock_admin_check_true, mock_databricks_admin_client, mock_db_for_metrics
):
  ...
  assert data['interval'] == '5min', (
    '24h range should use 5-minute intervals for higher granularity'
  )


def test_get_time_series_returns_hourly_interval_for_7d_and_longer(
  mock_admin_check_true, mock_databricks_admin_client, mock_db_for_metrics
):
...
def test_get_time_series_returns_403_for_non_admin(
  mock_admin_check_false, mock_databricks_non_admin_client
):
...
def test_get_usage_count_endpoint_returns_count(mock_databricks_admin_client, mock_db_for_metrics):
  ...
  """
  response = client.get(
    '/api/v1/metrics/usage/count?time_range=24h', headers={'X-Forwarded-Access-Token': 'mock-token'}
  )
