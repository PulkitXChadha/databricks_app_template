...
def test_is_workspace_admin_with_default_admin_groups():
  ...
  assert is_workspace_admin(user_info_workspace_admins) is True, (
    "Should detect 'workspace_admins' group"
  )
  ...
  assert is_workspace_admin(user_info_administrators) is True, (
    "Should detect 'administrators' group"
  )
...
def test_is_workspace_admin_with_custom_admin_groups():
  ...
  try:
    ...
    assert is_workspace_admin(user_info_custom) is True, (
      "Should detect custom 'custom_admins' group"
    )
...
def test_is_workspace_admin_case_insensitive_matching():
  ...
  for group_name in case_variations:
    user_info = {
      'id': '12345',
      'userName': 'admin@example.com',
      'groups': [{'display': group_name, 'value': 'group-1'}],
    }
...
def test_is_workspace_admin_handles_missing_groups():
  ...
  user_info_no_groups = {'id': '12345', 'userName': 'user@example.com'}
...
def test_is_workspace_admin_handles_empty_groups():
  ...
  user_info_empty_groups = {'id': '12345', 'userName': 'user@example.com', 'groups': []}
...
def test_is_workspace_admin_handles_missing_display_field():
  ...
  assert is_workspace_admin(user_info_malformed) is False, (
    'Should handle missing display field gracefully'
  )
...
def test_admin_check_prevents_privilege_escalation():
  ...
  injection_user_info = {
    'id': '12345',
    'userName': 'attacker@example.com',
    'groups': [{'display': "admins'; DROP TABLE users; --", 'value': 'group-1'}],
  }
