...
def mock_workspace_client():
  ...
  client.current_user.me.return_value = Mock(
    user_name='test@example.com', display_name='Test User', active=True
  )
  return client
...
def mock_user_auth(monkeypatch):
  ...
  # Patch the methods
  monkeypatch.setattr('server.services.user_service.UserService.get_user_info', mock_get_user_info)
  monkeypatch.setattr('server.services.user_service.UserService.get_user_id', mock_get_user_id)
  monkeypatch.setattr('server.lib.database.is_lakebase_configured', mock_is_lakebase_configured)
...
def sample_preferences():
  ...
  return [
    {'preference_key': 'theme', 'preference_value': {'color': 'dark', 'mode': 'auto'}},
    {'preference_key': 'language', 'preference_value': {'lang': 'en', 'region': 'US'}},
    {'preference_key': 'notifications', 'preference_value': {'email': True, 'push': False}},
  ]
...
def mock_admin_user():
  ...
  """
  return {'user_id': 'admin@example.com', 'email': 'admin@example.com', 'is_admin': True}
...
def mock_non_admin_user():
  ...
  """
  return {'user_id': 'user@example.com', 'email': 'user@example.com', 'is_admin': False}
...
def mock_databricks_admin_client():
  ...
  with patch('databricks.sdk.WorkspaceClient') as mock_client:
    ...
    mock_user.groups = [MagicMock(display='admins', value='group-admin-id')]
...
def mock_databricks_non_admin_client():
  ...
  with patch('databricks.sdk.WorkspaceClient') as mock_client:
    ...
    mock_user.groups = [MagicMock(display='users', value='group-users-id')]
...
def mock_admin_check_false():
  ...
  with patch(
    'server.services.admin_service.is_workspace_admin_async', side_effect=mock_is_not_admin
  ):
    yield
